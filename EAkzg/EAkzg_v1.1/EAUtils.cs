...
namespace EAkzg
{
    ...
    static class EAUtils
    {
      ...
      //  static String autor = "";
        ...
        static public String dajDostawceSystemu(EA.Repository rep, EA.Package p)
        {
       ...
       // static public void generujWklady(ref Repository repo, String pakiet,int obszar)
         static public void generujWklady(CModel modelProjektu, int obszar)
        {
            ...
            try
            {
             ...
             /* kzg nowy model   
                ...
                Package r = EAUtils.dajModelPR(ref repo); //repo.Models.GetAt(0);
              *
                Package rodzicPckg =utworzPakietGdyBrak(ref r,pakiet,"");
                Package wkladyPckg = utworzPakietGdyBrak(ref rodzicPckg, "Wkłady Systemowe","");
                Package systemyPckg = utworzPakietGdyBrak(ref rodzicPckg, "Architektura Statyczna", "");
                */
                        ...
                        // kzg nowy model    dodajWymaganieBezpieczenstwaDoSystemyGdyGoNieMa(ref repo,ref wymagPckg,pakiet);
                            ...
                            foreach (Connector c in element.Connectors)
                            {
                                if (c.Type == "Realisation")
                                {
                                    ...
                                    //interfejsElement = repo.GetElementByID(c.SupplierID);
                                    try
                                ...
                                if (c.Type == "Usage")
                                {
                                   // interfejsElement = repo.GetElementByID(c.ClientID);
                           ...
                           /* kzg nowy model
                            * projektInterfejs.LayoutDiagramEx(systemoCentrycznyDiagram.DiagramGUID, lay ,4, 20, 20, false);
                            systemoCentrycznyDiagram.Update();
                            repo.CloseDiagram(systemoCentrycznyDiagram.DiagramID);
                            * */
           ...
           //kzg nowy model     Package wymaganiaPckg = utworzPakietGdyBrak(ref r, "Wymagania", "");
        ...
        static private void zrobDiagramInterfejsocentryczny(/*kzg nowy model ref Repository repo*/ CModel modelProjektu,ref Package interfejsPckg, ref Element interfejsElem)
        {
            ...
            /*kzg nowy model
               EA.Project projektInterfejs = repo.GetProjectInterface();
               projektInterfejs.LayoutDiagramEx(interfejsoCentrycznyDiagram.DiagramGUID, lay, 4, 20, 20, false);
               */
        ...
        static private void rekuKopiaFiczer(CModel modelProjektu, ref Package rodzicPckg, ref Package p)
        {
            ...
            foreach (Element elem in p.Elements)
            {
                if (elem.Type == "Feature")
                {
                    ...
                    int ile_ficzerow = 0;
                    foreach (Connector cc in elem.Connectors)
                    {
                        try
                        {
                            Element e = modelProjektu.Repozytorium.GetElementByID(cc.ClientID);//repo.GetElementByID(cc.ClientID);
                           // Element e2 = repo.GetElementByID(cc.SupplierID);
                            if (e.Type == "Component")
                            {
                                ile_ficzerow++;
                            }
                    ...
                    if (ile_ficzerow == 0)
                    {
                       //kzg nowy model if (elem.Status != "Anulowane przez BO" && elem.Status != "Anulowane przez IT")
                        ...
                        {
                            MessageBox.Show("TODO rekuKopiaFiczer brak powiazania");
                        }
                    }
                    // jesli wiecej niz 1
         /*           if (ile_ficzerow > 1)
                    {
                        String duplikacjaStr = "Tworzenie kopii feature: " + elem.Name;
                        try
                        {
                            //// to zrob nowy element feature (nazwa jak poprzedni) 
                            Element nowyEl = EAUtils.dodajElement(ref pak, elem.Name, "", "Feature");
                            
                            nowyEl.Status = "Nowy";
                            nowyEl.Author = elem.Author;
                            nowyEl.Stereotype = elem.Stereotype;
                            nowyEl.Version = "1.0";
                            nowyEl.Update();
                            duplikacjaStr += "[ok]\n"+" * status-"+nowyEl.Status+"\n * autor-"+nowyEl.Author+"\n * stereotyp-"+nowyEl.Stereotype;
                            duplikacjaStr += "\n * wersja-" + nowyEl.Version + "\n ******** dodane realacje *****\n";
                            
                            //// dodaj do niego wszystkie konektory z oryginalu za wyjatkiem typu component
                         
                            Connector usuwany = null;
                            foreach (Connector cc in elem.Connectors)
                            {
                               
                                Element e = repo.GetElementByID(cc.ClientID);
                                if (e.Type != "Component")
                                {
                                    Element supl = repo.GetElementByID(cc.SupplierID);
                                    EAUtils.dodajRelacje(nowyEl, supl, cc.Type, cc.Name, cc.Notes);
                                    duplikacjaStr += " ->" + nowyEl.Name + " " + supl.Name + " typ: " + cc.Type + " nazwa: " + cc.Name + " notes: " + cc.Notes + "\n";
                                }
                                else
                                {
                            ...
                            //// dodaj konektor do systemu
                            Element supl1 = repo.GetElementByID(usuwany.SupplierID);
                            EAUtils.dodajRelacje(nowyEl, supl1, supl1.Type, "", "");
                            duplikacjaStr += " ->powiązanie systemowe" + nowyEl.Name + " " + supl1.Name + "\n";
   
                            
                            /// dodaj ficzer do diagramu
                            /// //// przenies ficzer do odpowiedniego pakietu
                            Package pakietSystPckg = dajPakietSciezkiP(ref rodzicPckg, "Wkłady Systemowe", supl1.Name, "Wymagania Systemowe");
                            // kopia wymagań do podfolderu systemowego
                            nowyEl.PackageID = pakietSystPckg.PackageID;
                            nowyEl.Update();

                            duplikacjaStr+="Przeniesiono "+nowyEl.Name +" do pakietu wymagań systemowych "+pakietSystPckg.Name+"\n";
                            duplikacjaStr += "\n Czy usunąć powiązanie z oryginału feature? \n";
                            
                            //// usun powiazanie pomiedzy starym ficzerem i  konektorem
                            if (MessageBox.Show(duplikacjaStr, "Rozmnożenie features zakończone, co teraz zrobić", System.Windows.Forms.MessageBoxButtons.YesNo) == DialogResult.Yes)
                            {
                                short x= 0;
                               for(short i=0 ;i<elem.Connectors.Count;i++)
                               {
                                    if (elem.Connectors.GetAt(i).ConnectorID == usuwany.ConnectorID)
                                    {
                                        x = i;
                                        break;
                                    }
                                }
                                elem.Connectors.DeleteAt(x,true);
                                elem.Update();
                            }
                        }
                        catch
                        {
                            ///błąd kopiowania/duplikowania feature
                        }
                    }
                    //jesli tylko 1 to go przenies po staremu
                    */
